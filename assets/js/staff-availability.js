jQuery(document).ready(function($) {
    ...
    // Get the staff ID from either the admin dropdown or the hidden input
    var staffId = $('#admin-selected-staff').val() || $('input[name="staff_post_id"]').val();

    console.log('Delete - Availability ID:', availabilityId, 'Staff ID:', staffId);
    
    // Additional debugging - FIXED: use sodAvailability instead of sodSchedulerAjax
    console.log('Ajax URL:', sodAvailability.ajax_url);
    console.log('Nonce element exists:', $('#sod-staff-availability-form input[name="sod_availability_nonce"]').length > 0);
    console.log('Nonce value:', $('#sod-staff-availability-form input[name="sod_availability_nonce"]').val());

    if (!staffId) {
        alert('Error: Unable to determine staff ID. Please refresh the page and try again.');
        return;
    }

    if (!availabilityId) {
        alert('Error: Unable to determine availability ID.');
        return;
    }
    ...
    if (confirm('Are you sure you want to delete this availability?')) {
        ...
        var ajaxData = {
            ...
            staff_post_id: staffId,
            nonce: $('#sod-staff-availability-form input[name="sod_availability_nonce"]').val()
        };
        
        console.log('Sending AJAX data:', ajaxData);
        
        $.ajax({
            url: sodAvailability.ajax_url,  // This line is already correct
            type: 'POST',
            data: ajaxData,
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    var $row = $('[data-availability-id="' + availabilityId + '"]').closest('tr');
                    $row.fadeOut(function() {
                        $row.remove();
                        // Check if table is now empty
                        if ($('.availability-table tbody tr').length === 0) {
                            $('.current-availability-list').html('<p class="no-availability">No availability slots found.</p>');
                        }
                    });
            ...
            error: function(xhr, status, error) {
                ...
                alert('An error occurred while deleting the availability. Check console for details.');
